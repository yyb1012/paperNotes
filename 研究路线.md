...
- 只进行**不变量掩码重构**,只保证了模型会补信息，不保证能够分开正负样本。
- 因此构造正负样本对，采用对比学习
### 2.1正负样本的构造
#### 正样本（保持不变量）
原始的良性子图
#### 负样本（打破不变量）
- 身份错配：将`writer`或`exec`的`(uid,gid,nx,exe_hash)`部分替换为另一主体，使得$v_{e-w}=1-sim(id(E),id(W))$ 升高
- 跨命名空间：令命名空间`mnt_ns/pid_ns/net_ns`与邻接事件不一致，触发$v_{ns}\uparrow$
- 时序逆序：交换相邻事件时间戳，使`Δt(write → exec)<0`, 触发$v_{time}\uparrow$
- 权限更改：在保持当前上下文的前提下，把主体的权限标签替换为更高的权限，触发$v_{priv}\uparrow$
- 路径错位：把“浏览器族”进程写入系统敏感路径等，触发$v_{path}\uparrow$
#### 批内策略
- 一个批次内包含同一主体（进程/文件）的正样本与多种“破坏不变量”的负样本 （来自 2.1 的五类：身份、命名空间、时序、权限、路径）。
- 优先采样违反程度高（$v_k$​ 大）、但语义相近（高 sim）的负样本，提升区分度
### 2.2 损失函数
- 计算表征向量 z
	- 先用时序编码器对时空子图编码，得到隐藏向量h
	- 再经过一个投影头得到对比空间的向量z
	- $z^-$来自打破不变量的负样本子图，$z^+$来自保持不变量的正样本子图
- InfoNCE损失函数
$$\mathcal{L}_{\text{nce}} = - \sum_i \log \frac{\exp(\tfrac{1}{\tau}\, s(\mathbf{z}_i,\mathbf{z}_i^+))}{\exp(\tfrac{1}{\tau}\, s(\mathbf{z}_i,\mathbf{z}_i^+)) + \sum_{j} \exp(\tfrac{1}{\tau}\, s(\mathbf{z}_i,\mathbf{z}_j^-))}$$
## 3. 不变量融合与异常检测头
### 输入
- 违例强度向量$v=(v_1,v_2,…,v_k)$
### 单调融合器
- 将多个违例强度融合成一个统一的 CIC 风险分数。
- 保证单调性：任意$v_k$增大，CIC 不下降
$$\text{CIC} = \sigma\!\left(W_2 \,\phi(W_1 *{v} + \mathbf{b}_1) + \mathbf{b}_2\right),
\quad W_1,W_2 \ge 0$$
- Topk池化：只取最大 k个$v_k$做加权，降低弱噪声影响
### 异常检测头
- 输入：CIC + 编码器嵌入特征h。
- 输出：通过MLP计算最终 anomaly score。$$SCORE=\sigma(w·[h_i||CIC_i]+b)$$
- 排序一致性损失（保证高 CIC → 高 anomaly）：
$$L_{\text{rank}} = \max(0, \, \text{margin} - ( \text{anomaly}_i - \text{anomaly}_j )),
\quad \text{if } \text{CIC}_i > \text{CIC}_j$$
## 4. 基于CIC的最小解释子图重构
